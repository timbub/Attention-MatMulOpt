...
namespace attention {
    ...
    tensor::Tensor<float> flash_attention(const tensor::Tensor<float>& Q,
        ...
        std::vector<float> Q_scaled(tilling_size * dk, 0.0f);
        ...
        for (size_t b = 0; b < batch_size; ++b) {
            ...
            for (size_t i0 = 0; i0 < seq_q; i0 += tilling_size) {
                size_t i_max = std::min(i0 + tilling_size, seq_q);
                ...
                for (size_t j0 = 0; j0 < seq_k; j0 += tilling_size) {
                    size_t j_max = std::min(j0 + tilling_size, seq_k);
